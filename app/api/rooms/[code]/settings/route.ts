...
export async function POST(
  ...
  try {
    ...
    const { gameSettings } = await req.json();
    if (!gameSettings || typeof gameSettings !== "object") {
      return NextResponse.json({ success: false, error: "Missing or invalid gameSettings" }, { status: 400 });
    }

    const res = await roomStore.updateGameSettings(code, sessionToken, gameSettings);
    if (!res.success) {
